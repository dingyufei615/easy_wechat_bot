...
app = FastAPI(title="api完整示例",
              description="########Tips: 先创建实例获得guid，再通过guid打开微信，登录后每次调用接口请携带上guid########")
...
@app.post("/client/open", summary="打开微信,当show_login_qrcode=True时, 打开微信时会显示二维码界面", tags=["Client"],
          response_model=models.ResponseModel)
@catch_exception()
